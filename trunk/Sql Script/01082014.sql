...
BEGIN
	...
	WHERE 
		...
		AND (@STATAUS = 0 OR (@STATAUS = 1 AND TRA_HANG_NCC.ID NOT IN (SELECT MA_PHIEU_TRA_NCC FROM XUAT_KHO WHERE XUAT_KHO.ACTIVE = 'A' AND CAST(XUAT_KHO.NGAY_XUAT AS DATE) >= CAST(@FROM_DATE AS DATE) AND XUAT_KHO.MA_PHIEU_TRA_NCC IS NOT NULL) ))

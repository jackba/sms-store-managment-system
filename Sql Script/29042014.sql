...
CREATE PROCEDURE STMA_GET_GIA_TRI_HANG_BAN_TON
@MA_KHO INT, 
@MA_SAN_PHAM INT, 
...
BEGIN
	...
	BEGIN
		...
		FETCH NEXT FROM SAN_PHAM_CURSOR INTO @ID_SAN_PHAM ,@TEN_SAN_PHAM, @TEN_DON_VI
	END
	SET @SO_LUONG_DA_XUAT = (SELECT DBO.STMA_GET_EXPORT_QUALITY(@NGAY_KIEM_KHO,@MA_KHO,@MA_SAN_PHAM ))
	...
	OPEN MY_IMPORT_CURSOR 
		...
		BEGIN   
				IF (@PCOUNT + @SO_LUONG_MUA) <= @SO_LUONG_DA_XUAT
				BEGIN
					SET @TONG_VON_HANG_BAN = @TONG_VON_HANG_BAN + @SO_LUONG_MUA*@DON_GIA_MUA
				END
				ELSE
				BEGIN
					IF @PCOUNT < @SO_LUONG_DA_XUAT
					BEGIN
						SET @TONG_VON_HANG_BAN = @TONG_VON_HANG_BAN + (@SO_LUONG_DA_XUAT - @PCOUNT)*@DON_GIA_MUA
					END
				END	
				IF (@PCOUNT + @SO_LUONG_MUA) > @SO_LUONG_DA_XUAT
				BEGIN
					IF @PCOUNT < @SO_LUONG_DA_XUAT
					BEGIN
						SET @TONG_VON_HANG_TON = @TONG_VON_HANG_TON + (@PCOUNT + @SO_LUONG_MUA -  @SO_LUONG_DA_XUAT)*@DON_GIA_MUA
					END
					ELSE
					BEGIN
						SET @TONG_VON_HANG_TON = @TONG_VON_HANG_TON + @SO_LUONG_MUA*@DON_GIA_MUA
					END
				END
				SET @PCOUNT = @PCOUNT + @SO_LUONG_MUA				
				FETCH NEXT FROM MY_IMPORT_CURSOR INTO @SO_LUONG_MUA ,@DON_GIA_MUA
